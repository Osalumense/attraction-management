...
@section('content')
    <div class="page-content">
        <div class="header h-12 px-10 py-8 my-4 border-b-2 border-gray-200 flex items-center justify-between">
            <div class="flex items-center space-x-2 text-gray-400">
                ...
                <span class="tracking-wide text-md ">
                    <span class="text-base">Add Attraction</span>
                </span>
                <span>/</span>
            </div>
            <div>
        ...
        <div class="header bg-white m-4 rounded-md p-4 h-12 px-10 flex items-center justify-center">
            <h1 class="font-medium text-2xl">Add Attraction</h1>
        </div>

        <div class="header bg-white m-4 rounded-md p-4 px-10 flex items-center justify-center">
            <form method="POST" action="{{ route('attractions.store') }}" enctype="multipart/form-data" class="mt-4">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-semibold mb-2" for="name">Name</label>
                    <input type="text" name="name" id="name" required class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Enter Attraction name">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Enter Attraction description"></textarea>
                    @error('description')
                        <span class="text-red-400">{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="text" name="location" id="location" required class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Enter Location">
                    @error('location')
                        <span class="text-red-400">{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input type="number" name="price" id="price" required class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Enter Price">
                    @error('price')
                        <span class="text-red-400">{{ $message }}</span>
                    @enderror
                </div>
